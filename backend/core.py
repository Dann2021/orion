...
models = [
    {
        "nom": "Projet",
        "attributs": {"nom": "string", "auteur": "string", "age": "integer"},
        "relations": [
            {
                "classe": "Projet",
            },
    ...
    {
        "nom": "Classe",
        "attributs": {"nom": "texte", "prix": "float"},
        "relations": [
            {
                "classe": "Projet",
            }
...
for model in models:
    ...
    nn = normalisation_model(model=model, type_mapping=TYPE_MAPPING, cible="attributs")
    bp = fabrique_blueprint(model=model)
    blue_print.append(bp)
    norme.append(nm)
    normes.append(nn)

type_valide = type_utilise(models)
extraire = extraire_type_utilise(models=models, type_mapping=TYPE_MAPPING)"""


# print("-----------------")
# print(f"liste Blueprint : {blue_print}\nNormes : {normes}\nTYPE_VALIDE : {type_valide}\nExtraire : {extraire}")


# nmd += normalize_relations(models=models)
# print(f"normalisation :  {nmd}")
